$(document).ready(function(){
    if(enviado){alert('Pedido de Venda Enviado.');}$('.botao input').mouseover(function(){$(this).css('cursor','pointer');}); $('#botoes .bnt,.outrosItens span').mouseover(function(){$(this).css('cursor','pointer');}); var item=1; var conf=null; var conf2=null;
    $('#botoes .bnt').click(function(){if(!$('.razao input').val()){alert('Preenche campo "CLIENTE"'); $('.razao input').focus(); return false;}else if(!$('.cnpj input').val()){alert('Preenche campo "CNPJ/CPF"'); $('.cnpj input').focus(); return false;}if($(this).text()=='Novo Ítem'){if(!$('#pnl1 table input[name=codigo_produto'+item+']').val()){$('#pnl1 table input[name=codigo_produto'+item+']').css('background','rgba(216, 0, 0,0.1)').focus(); busca(); $('#pnl1 table tr .lupa img[busca='+item+']').trigger('click'); die;}item++; $('#pnl1 table').append('<tr id=item'+item+'><span class=novo><td class="lupa" ></td><td class=item>'+item+'</td><td><input type="text" size=10px name="codigo_produto'+item+'" autocomplete="off" linha='+item+' required/></td><td><input type="text" name="descricao'+item+'" autocomplete="off" required/></td><td><input type="text" size=2px name="quantidade'+item+'" autocomplete="off" required linha='+item+'/></td><td><input type="text" size=8px name="vUnitarioItem'+item+'" autocomplete="off" required/></td><td><input type="text" size=8px name="vTotalItem'+item+'" linha='+item+' disabled/></td><td><input type="text" size=2px name="pDescontoItem'+item+'" autocomplete="off" linha='+item+'/></td><td><textarea cols=25 rows=1 name="obs_item'+item+'" linha='+item+'/></textares></td><td><input type="hidden" name="cfop'+item+'" linha='+item+'/></td><td><input type="hidden" name="ncm'+item+'" linha='+item+'/></td><td><input type="hidden" name="ean'+item+'" linha='+item+'/></td><td><input type="hidden" name="unidade'+item+'" linha='+item+'/></td><td><input type="hidden" name="vTotalItem'+item+'" linha='+item+'/></td><td><input type="hidden" name="pTabela'+item+'" linha=1/></td><input type=hidden name="cOmie'+item+'"/></span></tr>'); itemExclusao(); busca(); $('#pnl1 table input[name=codigo_produto'+item+']').trigger('focus');}else if($(this).text()=='Excluir Ítem'){if($('#pnl1 table input').val()){alert('Clique no Nº do Ítem para excluir.'); if(!conf){itemExclusao();}}}});busca();
    $('.outrosItens span').click(function(){$(this).css({position: 'relative', top: '1px', boxShadow: 'none', background: '#b1aea1', color: 'white', textShadow: '2px 2px 2px black'}); var bntAtual=$(this).attr('class'); $('.outrosItens span').each(function(){if(!$(this).hasClass(bntAtual)){$(this).css({position: 'relative', top: '-1px', background: '#d4d2c1', boxShadow: '0 2px 0 gray', textShadow: '2px 2px 2px white', color: 'black'});}}); $('#pnl1, #pnl2, #pnl3, #pnl4, #pnl5, #pnl6').css('display','none'); if($(this).text()==='Itens de Venda'){$('#pnl1').css('display','block');}else if($(this).text()==='Frete/Despesas'){$('#pnl2').css('display','block');}else if($(this).text()==='Informações Adcionais'){$('#pnl3').css('display','block');}else if($(this).text()==='Parcelas'){$('#pnl4').css('display','block'); $('.tabela select[name=parcela]').removeAttr('disabled');}else if($(this).text()==='E-mail para Cliente'){$('#pnl5').css('display','block');}})
    $('table.tabela .razao img').click(function(){if($('div#pnl1 input').val()){var resp=confirm('Você já adcionou itens a este pedido, estes dados serão descartados. Deseja continuar?'); if(resp){$(location).attr('href','index.php?pagina=cliente&act=list&seleciona=1&tags=undefined');}else{return false;}}else{$(location).attr('href','index.php?pagina=cliente&act=list&seleciona=1&tags=undefined');}})
    $(document).keydown(function(e){if(e.which==113){if($('form').hasClass('#pnl1') || $('#pnl1 input').val()){var resp=confirm('Você já adcionou itens a este pedido, estes dados serão descartados. Deseja continuar?'); if(resp){$(location).attr('href','index.php?pagina=cliente&act=list&seleciona=1&tags=undefined');}else{return false;}}else{$(location).attr('href','index.php?pagina=cliente&act=list&seleciona=1&tags=undefined');}}else if(e.which==112 || e.which==114 || e.which==116 || e.which==123){return false;}else if(e.which==115){$('.bnt').each(function(){if($(this).text()=='Novo Ítem'){$(this).trigger('click');}})}else if(e.which==27){$("#mascara").hide(); $(".window").hide();}else if(e.which==119){$('.procura img').trigger('click'); return false;}})
    $('.tabela td select[name=parcela]').change(function(){$('.outrosItens span').each(function(){if($(this).text()=='Parcelas'){$(this).trigger('click');}}); var numParc=$('.tabela select[name=parcela] :selected').val(); var formPag=$('.tabela td select[name=fPagamento] :selected').val();
        if(formPag=='debito' || formPag=='dinheiro'){if(numParc !='A Vista,000,0'){$('.tabela select[name=parcela] option').each(function(){$(this).removeAttr('selected'); if($(this).val()=='A Vista,000,0'){$(this).attr('selected','selected');}}); $('#pnl4 .novo4').each(function(){if($(this).attr('linha')>1){$(this).remove();}}); $('#pnl4 .novo4 input[name=numero_parcela1]').val(0); $('#pnl4 .novo4 input[name=data_vencimento1]').val($('.previsao input[name=dPrevisao]:hidden').val()); $('#pnl4 .novo4 input[name=valor1]').val($('.autoQuadro td[nome=vPedido]').text()); $('#pnl4 .novo4 input[name=percentual1]').val(100);}}else if(formPag=='credito'){if(numParc=='A Vista,000,0' || numParc==''){$('.tabela select[name=parcela] option').each(function(){$(this).removeAttr('selected'); if($(this).val()=='1 Parcela,001,1'){$(this).attr('selected','selected');}}); $('#pnl4 .novo4').each(function(){if($(this).attr('linha')>1){$(this).remove();}}); $('#pnl4 .novo4 input[name=numero_parcela1]').val(1); $('#pnl4 .novo4 input[name=data_vencimento1]').val(data(1)); $('#pnl4 .novo4 input[name=valor1]').val($('.autoQuadro td[nome=vPedido]').text()); $('#pnl4 .novo4 input[name=percentual1]').val(100);
            }else{
    ...
    function preencheTotais(){
            ...
            $('.autoQuadro td[nome=tItem]').text(item);
            ...
            if(mercadoriaTotal){
                ...
                $('.autoQuadro td[nome=mercadorias]').text(numeroParaMoeda(mercadoriaTotal));
            }
            if(vDescontoTotal){
                $('.autoQuadro input[name=vDesconto]').val(numeroParaMoeda(vDescontoTotal));
                $('.autoQuadro td[nome=vDesconto]').text(numeroParaMoeda(vDescontoTotal));
            }else{
            ...
            $('.autoQuadro td[nome=vPedido]').text(numeroParaMoeda(vPedido));
