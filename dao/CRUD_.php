<?php 
 class CRUD extends Dao{
   public function insert(Model $model){
                ...
                $sql = 'INSERT INTO tb_cliente (`modificado`,`inscricao_suframa`,`produtor_rural`,`recomendacao_atraso`,`logradouro`,`importado_api`,`bloqueado`,`bairro`,`cep`,`cidade`,`cidade_ibge`,`cnae`,`cnpj_cpf`,`codigo_cliente_integracao`,`codigo_cliente_omie`,`codigo_pais`,`complemento`,`email`,`endereco`,`endereco_numero`,`estado`,`exterior`,`cImpAPI`,`dAlt`,`dInc`,`hAlt`,`hInc`,`uAlt`,`uInc`,`info`,`inscricao_estadual`,`inscricao_municipal`,`nome_fantasia`,`pessoa_fisica`,`razao_social`,`tags`,`telefone1_ddd`,`telefone1_numero`,`tipo_atividade`,`cod_API`,`contato`,`optante_simples_nacional`,`telefone2_ddd`,`telefone2_numero`,`fax_ddd`,`fax_numero`,`homepage`,`observacao`,`contribuinte`,`id`,`excluido`,`criado`) VALUES (:modificado,:inscricao_suframa,:produtor_rural,:recomendacao_atraso,:logradouro,:importado_api,:bloqueado,:bairro,:cep,:cidade,:cidade_ibge,:cnae,:cnpj_cpf,:codigo_cliente_integracao,:codigo_cliente_omie,:codigo_pais,:complemento,:email,:endereco,:endereco_numero,:estado,:exterior,:cImpAPI,:dAlt,:dInc,:hAlt,:hInc,:uAlt,:uInc,:info,:inscricao_estadual,:inscricao_municipal,:nome_fantasia,:pessoa_fisica,:razao_social,:tags,:telefone1_ddd,:telefone1_numero,:tipo_atividade,:cod_API,:contato,:optante_simples_nacional,:telefone2_ddd,:telefone2_numero,:fax_ddd,:fax_numero,:homepage,:observacao,:contribuinte,:id,:excluido,:criado)';
  ...
  public function getParams(Model $model){
    ...
    public function insert3(Model $model){
        ...
        $sql = 'INSERT INTO tb_vendedor (`modificado`,`codigo`,`codInt`,`nome`,`inativo`,`comissao`,`email`,`fatura_pedido`,`visualiza_pedido`,`id`,`excluido`,`criado`) VALUES (:modificado,:codigo,:codInt,:nome,:inativo,:comissao,:email,:fatura_pedido,:visualiza_pedido,:id,:excluido,:criado)';
	$search = new ModelSearchCriteria();
        $search->settabela($model->gettabela());
    ...
    public function update3(Model $model){
        ...
        $now = mktime (date("H"), date("i"), date("s"), date("m")  , date("d"), date("Y"));
        $model->setmodificado($now);
        $sql = 'UPDATE tb_vendedor SET id=:id,criado=:criado,modificado=:modificado, modificado=:modificado,codigo=:codigo,codInt=:codInt,nome=:nome,inativo=:inativo,comissao=:comissao,email=:email,fatura_pedido=:fatura_pedido,visualiza_pedido=:visualiza_pedido,id=:id,excluido=:excluido,criado=:criado WHERE id = :id ';
        return $this->execute3($sql, $model);
    }
    public function getParamsVend(Model $model){
        $params=array(':id'=>$model->getid(),':excluido'=>$model->getexcluido(),':criado'=>$model->getcriado(),':modificado'=>$model->getmodificado(),':codigo'=>$model->getcodigo(),':codInt'=>$model->getcodInt(),':nome'=>$model->getnome(),':inativo'=>$model->getinativo(),':comissao'=>$model->getcomissao(),':email'=>$model->getemail(),':fatura_pedido'=>$model->getfatura_pedido(),':visualiza_pedido'=>$model->getvisualiza_pedido());
    }
    public function criaTabela3($tabela){
        $sql="CREATE TABLE IF NOT EXISTS tb_vendedor ( `id` INT(5) NOT NULL AUTO_INCREMENT , `criado` INT(100) NULL,`modificado` varchar(100) NULL,`codigo` varchar(100) NULL,`codInt` varchar(100) NULL,`nome` varchar(100) NULL,`inativo` varchar(100) NULL,`comissao` varchar(100) NULL,`email` varchar(100) NULL,`fatura_pedido` varchar(100) NULL,`visualiza_pedido` varchar(100) NULL, `excluido` ENUM('0','1') NOT NULL DEFAULT '0', PRIMARY KEY (`id`)) ENGINE = InnoDB CHARACTER SET utf8 COLLATE utf8_general_ci";
        return $sql;
    }
}